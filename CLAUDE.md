...
1. `/analyze-my-resume` will set `isAgentic: true` in the manifest, activate the agentic graph lanes (11 and 12), run the 20-point Agentic Design Estimates Checklist inside the design-estimates lane, and produce `12-agentic-graph-structure.md`, `13-memory-layer-design.md`, `14-ingestion-pipeline.md` (if `hasKnowledgeBase: true`), and `15-guardrails.md` alongside the standard file set.
2. **In-loop critic checkpoint (automatic, agentic only).** After the parallel batch completes and *before* Lane 12 (per-node state) or Lane 10 (challenges) run, `/analyze-my-resume` automatically spawns an in-loop critic against `03-architecture.md`, `12-agentic-graph-structure.md` (Layer 1), `13-memory-layer-design.md`, `14-ingestion-pipeline.md` (if applicable), and `15-guardrails.md`. Lanes with BLOCKING verdicts revise and re-critique (cap: 2 cycles per file). Surviving objections land in `00-question-and-context.md` under `## Surviving Critic Objections`. This catches structural flaws while reshaping is still cheap - before Layer 2 commits to the topology and challenges consume the pack.
3. After the pack is written, the user may still invoke `/critical-agent` to run the full three-phase production-readiness gate. The in-loop checkpoint is design-time; `/critical-agent` is the post-hoc approval gate that produces `20-critical-agent-approval.md`.
4. The approval artifact (`20-critical-agent-approval.md`) is only written by `/critical-agent` - never by hand.
