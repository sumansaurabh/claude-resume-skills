...
makes `19-agentic-graph-structure.md` and `20-memory-layer-design.md` required files.
...
`22-ingestion-pipeline.md` — the write-path design for the document ingestion and
...
- `22-ingestion-pipeline.md` *(required only when `hasKnowledgeBase: true`)*:
  ...
  Checklist). The embedding model named here must match `20-memory-layer-design.md`
...
- `21-critical-agent-approval.md` *(written by `/critical-agent` only)*: the
