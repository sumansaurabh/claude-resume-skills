...
| 19 | OTel trace per run, span per node, hop-counter metric, time-in-node histogram | ClickHouse spans sized in §5.4 |
| 20 | Peak = 50K concurrent runs (5% of 1M MAU in 30-min peak); fan-out 3 (supervisor → ~3 specialists); supervisor concurrency = bottleneck; size for 10K supervisor RPS | Drives full fleet in §5.4 |
